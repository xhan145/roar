...
def _previous_published():
    """Published column of the existing VERSIONS.md, keyed by component name.
    An offline run (the pre-commit hook) must not erase what the last --github
    run learned — otherwise every commit dirties the dashboard."""
    text = _read(DESKTOP, "VERSIONS.md") or ""
    out = {}
    for line in text.splitlines():
        cells = [c.strip() for c in line.split("|")]
        if len(cells) >= 4 and cells[1] and not cells[1].startswith(("-", "Component")):
            out[cells[1]] = cells[3]
    return out
...
def write_dashboard(results):
    ...
    for r in results:
        rel = r.get("release")
        if rel in (None, "unknown"):        # not queried / lookup failed
            rel_kept = previous.get(r["name"])
            pub = rel_kept if rel_kept and rel_kept != "—" else (
                "? (offline)" if rel == "unknown" else "—")
        else:
            pub = {"none": "❌ none"}.get(rel, rel)
        if not r["found"]:
